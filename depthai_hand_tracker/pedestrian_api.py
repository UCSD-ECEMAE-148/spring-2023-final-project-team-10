...
with dai.Device() as device:
    ...
    while True:
        ...
        if msgs is not None:
            ...
            #break
        hc.now = monotonic()
        frame, hands, bag = hc.tracker.next_frame()
        if frame is None: break
        hc.frame_nb += 1
        events = hc.generate_events(hands)
        hc.process_events(events)
        if hc.use_renderer:
            frame = hc.renderer.draw(frame, hands, bag)
            key = hc.renderer.waitKey(delay=1)
